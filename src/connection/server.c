...
{
    struct server_connection *http_server = server->data;
...
static void _tcp_on_data(struct tcp_server *server, socket_t sockfd)
{
    ...
    struct client_connection *client = map_get(&http_server->clients, &sockfd, sizeof(sockfd));
    ...
    if ((result = http_server_parse_request(http_server, client, &current_state)) != 0)
    {
        if (result < 0)
        {
            ...
            if (http_server->on_malformed_request)
            {
                ...
                http_server->on_malformed_request(http_server, client, result);
            }
        };
    ...
    // void (*callback)(struct server_connection *server, struct client_connection *client, struct http_request request) = http_server_find_route(http_server, path);
    struct server_route *route = http_server_find_route(http_server, path);
    if (route == NULL)
    {
        // That comedian...
    ...
    switch (route->connection_type)
    {
        case CONNECTION_HTTP:
        {
            void (*callback)(struct server_connection *server, struct client_connection *client, struct http_request request) = route->http_callback;
            if (callback != NULL) callback(http_server, client, current_state.request);
            break;
        };
        case CONNECTION_WS:
        {
            void (*callback)(struct server_connection *server, struct client_connection *client, struct ws_message message) = route->ws_callback;
            if (callback != NULL) callback(http_server, client, /** todo */ (struct ws_message){});
            break;
        };
    };
...
{
    vector_init(&http_server->routes, 8, sizeof(struct server_route));
...
{
    struct server_route *return_route = NULL;

    for (size_t i = 0; i < server->routes.size; ++i)
    {
        struct server_route *route = vector_get(&server->routes, i);
...
int http_server_send_response(struct server_connection *server, struct client_connection *client, struct http_response *response, const char *data, size_t data_length)
{
    socket_t sockfd = client->client->sockfd;
